...
const Player = (props) => {
  ...
  useEffect(() => {
    getSongUrl(props.currentSong.name || props.currentSong.songname).then(
      (playerUrl) => {
  ...
  useEffect(() => {
    ...
    getLynic(props.currentSong.name || props.currentSong.songname).then(
      (res) => {
        ...
        !!lyricListRef.current && lyricListRef.current.scrollTo(0, 0, 1000);
      }
    );

    if (lyricListRef.current) {
      lyricListRef.current.wrapperRef.current.style[
        transform
      ] = `translate3d(0, 0, 0)`;
  ...
  const handleLyric = useCallback(
    ({ lineNum = 0, txt }) => {
      ...
      if (lineNum > 5) {
        let lineEl =
          !!lyricLineRef.current && lyricLineRef.current.children[lineNum - 5];
  ...
  const middleTouchEnd = useCallback(
    (e) => {
      ...
      if (lyricListRef.current) {
        ...
        lyricListRef.current.wrapperRef.current.style[transitionDuration] =
          "300ms";
      }
  ...
  const _setMusicReadyState = useCallback(
    (props) => {
      dispatch({
        type: "set_song_ready",
        payload: props,
  ...
  return (
    <div className="player">
        ...
        <div className="normal-player">
          ...
          <div className="bottom">
            ...
            >
              <ProgressBar
                percent={percentage}
                percentageChanged={(value, isMoveAction) =>
                  percentageChanged(value, isMoveAction)
                }
              />
            </Cover>
      ...
      {!!showPlayList && (
        <PlayList hidePlayList={() => setShowPlayList(!showPlayList)} />
      )}
      <audio
